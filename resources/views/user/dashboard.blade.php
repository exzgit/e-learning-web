...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="w-full flex flex-col gap-4">
        ...
        @if (Request::is('dashboard'))
            <a href="/dashboard" class="bg-blue-400 w-full px-4 py-2 rounded-md font-medium text-white flex items-center gap-2">
                <i data-feather="home" class="w-5 h-5"></i> Dashboard
            </a>
        @else
            <a href="/dashboard" class="bg-white w-full px-4 py-2 rounded-md font-medium text-black flex items-center gap-2">
                <i data-feather="home" class="w-5 h-5"></i> Dashboard
            </a>
        @endif

        @if (Request::is('user/materi/view') || Route::is('user.materi.detail'))
            <a href="/user/materi/view" class="bg-blue-400 w-full px-4 py-2 rounded-md font-medium text-white flex items-center gap-2">
                <i data-feather="book-open" class="w-5 h-5"></i> Materi
            </a>
        @else
            <a href="/user/materi/view" class="bg-white w-full px-4 py-2 rounded-md font-medium text-black flex items-center gap-2">
                <i data-feather="book-open" class="w-5 h-5"></i> Materi
            </a>
        @endif

        @if (Request::is('user/proyek') || Route::is('user.upload.kerja.proyek'))
            <a href="/user/proyek" class="bg-blue-400 w-full px-4 py-2 rounded-md font-medium text-white flex items-center gap-2">
                <i data-feather="briefcase" class="w-5 h-5"></i> Proyek
            </a>
        @else
            <a href="/user/proyek" class="bg-white w-full px-4 py-2 rounded-md font-medium text-black flex items-center gap-2">
                <i data-feather="briefcase" class="w-5 h-5"></i> Proyek
            </a>
        @endif

        @if (Request::is('user/kelompok') || Route::is('user.kuis.kerja'))
            <a href="/user/kelompok" class="bg-blue-400 w-full px-4 py-2 rounded-md font-medium text-white flex items-center gap-2">
                <i data-feather="users" class="w-5 h-5"></i> Kelompok Kerja
            </a>
        @else
            <a href="/user/kelompok" class="bg-white w-full px-4 py-2 rounded-md font-medium text-black flex items-center gap-2">
                <i data-feather="users" class="w-5 h-5"></i> Kelompok Kerja
            </a>
        @endif

        @if (Request::is('user/penilaian'))
            <a href="/user/penilaian" class="bg-blue-400 w-full px-4 py-2 rounded-md font-medium text-white flex items-center gap-2">
                <i data-feather="check-circle" class="w-5 h-5"></i> Penilaian & Evaluasi
            </a>
        @else
            <a href="/user/penilaian" class="bg-white w-full px-4 py-2 rounded-md font-medium text-black flex items-center gap-2">
                <i data-feather="check-circle" class="w-5 h-5"></i> Penilaian & Evaluasi
            </a>
        @endif
    </div>
